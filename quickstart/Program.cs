...
var profile = new KitProfile(
    ...
    "Pick one trusted drawing package, collect accepted output examples, then port only the repeated checks into AutoLISP or AutoCAD .NET.",
    new string[] { "ssget", "entget", "tblsearch", "entmod", "command", "Document", "Database", "Transaction", "BlockTable", "BlockTableRecord", "LayerTable", "DBText", "AttributeReference" },
    new string[] { "Order or workflow request", "DXF/DWG fixture inventory", "Layer and text validation", "AutoLISP command decision", "AutoCAD .NET adapter decision", "Drawing package report", "Drafter review", "Accepted or corrected output" },
    new[]
    {
        new FixtureSpec("fixtures/public/gdal/assorted.dxf", "DXF", "Entity and layer smoke fixture.", "OSGeo GDAL test data", "MIT-style GDAL project sample", new string[] { "SECTION", "ENTITIES" }),
        new FixtureSpec("fixtures/public/gdal/text.dxf", "DXF", "Text entity validation fixture.", "OSGeo GDAL test data", "MIT-style GDAL project sample", new string[] { "TEXT" }),
        new FixtureSpec("fixtures/public/gdal/line_r2000.dwg", "DWG", "DWG package-presence fixture; geometry inspection remains AutoCAD-runtime gated.", "OSGeo GDAL test data", "MIT-style GDAL project sample", Array.Empty<string>()),
    },
    new[]
    {
        new ParetoRule("Layer and entity inventory", "Stops a script rescue from changing drawings before the team knows which layers and entities are actually in scope.", "AutoLISP `tblsearch` / `ssget`, then AutoCAD .NET `LayerTable` and `BlockTableRecord`.", new string[] { "SECTION", "ENTITIES" }),
        new ParetoRule("Text and title-block surface", "Catches the repeated note/title-block checks that usually create manual cleanup.", "AutoLISP `entget`, AutoCAD .NET `DBText`, `MText`, and `AttributeReference`.", new string[] { "TEXT" }),
        new ParetoRule("DWG native runtime gate", "Prevents false confidence from public parsers when full AutoCAD behavior, CTB/STB, xrefs, fonts, or COM behavior matters.", "Licensed AutoCAD desktop worker, AutoLISP routine, or managed .NET command.", new string[] { "DWG" }),
    });
